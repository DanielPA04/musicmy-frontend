...
export class SharedHomeRoutedComponent implements OnInit {
  ...
  getNew() {
    this.oAlbumService
      .getPageNew(this.nPage, this.nRpp, this.strField, this.strDir)
      .subscribe({
  ...
  getTopRated() {
    this.oAlbumService
      .getPageTopRated(this.nPage, this.nRpp, this.strField)
      .subscribe({
        next: (oPageFromServer: IPage<IAlbum>) => {
  ...
  getPopular() {
    this.oAlbumService
      .getPagePopularRecent(this.nPage, this.nRpp, this.strField)
      .subscribe({
        next: (oPageFromServer: IPage<IAlbum>) => {
  ...
  navegarRecent() {
    this.oRouter
      .navigate(['album'], { queryParams: { mode: 'recent' } })
      .then(() => {
  ...
  navegarTopRated() {
    this.oRouter
      .navigate(['album'], { queryParams: { mode: 'top-rated' } })
      .then(() => {
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });
  }
  ...
  navegarPopular() {
    this.oRouter
      .navigate(['album'], { queryParams: { mode: 'popular' } })
      .then(() => {
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });
  }
